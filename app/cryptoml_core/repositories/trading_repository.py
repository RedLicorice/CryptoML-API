...
class BaselineRepository(DocumentRepository):
    __collection__ = 'baselines'
    __model__ = Baseline

    def get_by_symbol(self, name: str, target: str, symbol: str):
        query = {"name": name, "target": target, "symbol": symbol,}
        document = self.collection.find_one(query)
        if not document:
            return None
        return self.__model__.parse_obj(document)

    def create_by_symbol(self, name: str, target: str, symbol: str, fiat: float, balance: Optional[float] = 0.0):
        a = Baseline(
            name=name,
            target=target,
            symbol=symbol,
